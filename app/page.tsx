...
// --- Helper Icon Components (replaces react-icons) ---
const UsersIcon = () => (
...
export default function SkillDashHome() {
  // Expanded list of skills for the rolling text animation
  const skills = [
    "Digital Marketing", "Graphic Design", "Web Development", "Content Creation", "Public Speaking",
    "Data Analysis", "Project Management", "UI/UX Design", "Financial Modeling", "Creative Writing",
    "Video Editing", "Ethical Hacking", "SEO Optimization", "Critical Thinking", "Leadership",
  ];
  // We add the first skill again at the end to ensure a seamless loop
  const loopingSkills = [...skills, skills[0]];

  // Core features of the SkillDash platform with corrected image paths
  const coreFeatures = [
    {
  ...
  return (
    <>
      <style jsx global>{`
        /* Keyframes for the rolling text animation */
        @keyframes scroll-words {
          0% { transform: translateY(0); }
          ${loopingSkills.map((_, index) => {
            if (index === 0) return '';
            const step = 100 / skills.length;
            const start = step * index - (step / 2);
            const end = step * index + (step / 2);
            return `${start}%, ${end}% { transform: translateY(-${(100 / loopingSkills.length) * index}%); }`;
          }).join('\n')}
        }
        .animate-scroll-words {
          /* The duration should be proportional to the number of skills */
          animation: scroll-words ${skills.length * 1.5}s ease-in-out infinite;
        }
        .scrolling-text-mask {
        ...
        <div className="max-w-7xl mx-auto px-4 py-24 sm:py-32">
          ...
          <section className="text-center mb-24 sm:mb-32">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 text-2xl sm:text-3xl font-semibold mb-10">
              ...
              <div className="relative h-12 w-64 overflow-hidden scrolling-text-mask text-left">
                <div className="animate-scroll-words">
                  {loopingSkills.map((skill, index) => (
                    <div key={index} className="h-12 flex items-center bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-transparent">
                      {skill}
                    </div>
                  ))}
                </div>
