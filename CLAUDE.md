...
- **Not yet built:** Scraper B (the lankabd.com price failsafe that activates only when the primary DSE price scraper's heartbeat goes stale) and the Resend failover-notification wiring described in the original pipeline plan — this section covers Scraper A (sector mapping) only.
