...
- **Every pushed commit must be numbered**, on `main` or `preview`, no exceptions — prefix the commit message with a version number, e.g. `3.5.5 feat: ...` / `3.5.6 fix: ...`. Find the next number with `git log --oneline --all | grep -oE '^[a-f0-9]+ [0-9]+\.[0-9]+\.[0-9]+' | head -1` and increment the patch digit. `package.json`'s own `"version"` field is not this number and isn't kept in sync — the numbering lives in commit messages only.
